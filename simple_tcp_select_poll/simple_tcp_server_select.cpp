...
int main(int argc, char *argv[]) {
  ...
  tv.tv_sec = 2;
  tv.tv_usec = 0;
    //vector of pointers to clients to loop over to see if the client has sent anything
  ...
  while (true) {
    ...
    FD_ZERO(&write_set);
    FD_ZERO(&except_set);
    //set tcp socket, the only thing we do repeatedly on this socket is accept
    //when set a new client is here
    ...
    for (int i = 0; i < tcp_clients.size(); i++) {
      ...
      //get notified when any new data comes in from a client
    ...
    if (FD_ISSET(tcp_socket, &read_set)) { //the tcp socket is special, only used to accept new clients, declare client, fill it out
    ...
    for (int i = 0; i < tcp_clients.size(); i++) {
      ...
      if (FD_ISSET(tcp_clients[i]->sock_fd, &read_set)) {
        ...
        //ready in data not processing it
        if (ret == -1) {
          perror("recv");
        } else if (ret == 0) { // Socket shutdown, client has closed the connection
      ...
      if (FD_ISSET(tcp_clients[i]->sock_fd, &write_set)) { //
        ...
        if (tcp_clients[i]->bytes_to_send > 0) { //if data is in send buffer we send it out now
...
//iterate over all clients, check recv_buf?
//handle_rat
  close(client_socket);
  close(tcp_socket);
  return 0;
